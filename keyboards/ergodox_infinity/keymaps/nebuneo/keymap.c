...
#define R_KC_CAPS F(M_KC_CAPS_R)

// timers for tap/hold
static uint16_t m_kc_caps_l_timer;
static uint16_t m_kc_caps_r_timer;
...
const uint16_t PROGMEM fn_actions[] = {
	...
	[M_KC_CAPS_R] = ACTION_MACRO_TAP(M_KC_CAPS_R),
};
...
const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
	...
	switch(id) {
		...
		case M_KC_CAPS_R:
			if (record->event.pressed) {
				m_kc_caps_r_timer = timer_read();

				if (!tap_not_interrupted) { //set mod
					return MACRO(D(CAPS), END);
				}
			} else {
				if (tap_not_interrupted && timer_elapsed(m_kc_caps_r_timer) < TAPPING_TERM) { // send tap-character
					record->tap.count = 0;
